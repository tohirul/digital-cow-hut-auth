...
const createCowProfile = async payload => {
  ...
  return await cow_model_1.default
    .findById({ _id: cowProfile._id })
    .populate('seller');
};
/**
...
const getAllCowProfile = async (filters, paginationOptions) => {
  ...
  const { page, limit, skip, sortBy, sortOrder, maxPrice, minPrice } = (0,
  pagination_helper_1.default)(paginationOptions);
  ...
  // Perform the query
  const result = await cow_model_1.default
...
const deleteCowProfile = async id => {
  ...
  session.startTransaction();
  try {
    const deletedProfile = await cow_model_1.default
